...
if __name__ == '__main__':
    topn = 1
    corpora = ['gov2', 'antique', 'dbpedia', 'clueweb09b']
    if not corpora:
        corpora = ['aol-ia']
    if not topn:
        topn = 1
    refiner = ['nllb', 'refiner', 'bt']
    rankers = ['bm25', 'qld']
    metrics = ['map']
    for corpus in corpora:
        for ranker in rankers:
            ...
            for metric in metrics:
                for ref in refiner:
                    df = pd.read_csv(f'{ReQue["input"]}/{corpus}/{ranker}.{metric}/{ranker}.{metric}.boxes/platinum.{ref}.tsv', sep='\t', names=['qid', 'query', 'map', 'query_', 'map_'])
                    tsv2json(df, f'{ReQue["output"]}/{corpus}/{ranker}.{metric}.{ref}/', topn)
                    data_dir = f'{ReQue["output"]}/{corpus}/{ranker}.{metric}.{ref}/'
