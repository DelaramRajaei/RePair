...
sys.path.extend(["./cair", "./cair/main", "./cair/neuroir"])
    ...
    for corpus in corpora:
        for ranker in rankers:
            ...
            for metric in metrics:
                ...
                if corpus == 'aol-ia':
                    df = pd.read_csv(f'{ReQue["input"]}/{corpus}/{ranker}.{metric}/{ranker}.{metric}.boxes/platinum.tsv', sep='\t', names=['qid', 'query', 'map', 'query_', 'map_'])
                    tsv2json(df, f'{ReQue["output"]}/{corpus}/{ranker}.{metric}.{refiner}/', topn)
                data_dir = f'{ReQue["output"]}/{corpus}/{ranker}.{metric}.{refiner}/'
                ...
                call_cair_run(data_dir, epochs=[2])
